...
router.post('/', contentRequired, upload.single('image'), (req, res) => {
  const { title, category, excerpt, content, image_url, active } = req.body || {};
  if (!title) return res.status(400).json({ error: 'Le titre est requis.' });
  const image = req.file ? `/uploads/${req.file.filename}` : (image_url || null);
  const info = db.prepare(
    'INSERT INTO posts (title, category, excerpt, content, image, active) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(title, category || null, excerpt || null, content || null, image, parseActive(active));
...
router.put('/:id', contentRequired, upload.single('image'), (req, res) => {
  ...
  db.prepare(
    'UPDATE posts SET title=?, category=?, excerpt=?, content=?, image=?, active=? WHERE id=?'
  ).run(
